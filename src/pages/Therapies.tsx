...
import { Brain, Sparkles, Sprout, Users, Smile, Paintbrush } from "lucide-react";
...
const therapyTypes = [{
  ...
  benefits: ["Resultados em um período relativamente curto", "Abordagem prática e orientada para soluções", "Desenvolvimento de habilidades que podem ser usadas ao longo da vida", "Eficácia comprovada para ansiedade, depressão e outros transtornos"],
  idealFor: ["Ansiedade", "Depressão", "Transtornos de humor", "Fobias", "Vícios", "Estresse"]
}, {
  id: "2",
  name: "Psicanálise",
  ...
  benefits: ["Autoconhecimento profundo", "Compreensão das motivações inconscientes", "Resolução de padrões repetitivos de comportamento", "Transformação duradoura da personalidade"],
  idealFor: ["Transtornos de personalidade", "Questões de identidade", "Trauma profundo", "Relacionamentos disfuncionais", "Angústia existencial"]
}, {
  id: "3",
  ...
  benefits: ["Desenvolvimento de autenticidade", "Melhora na autoestima e autoaceitação", "Ambiente terapêutico acolhedor e não-julgador", "Fortalecimento da autonomia e responsabilidade pessoal"],
  idealFor: ["Baixa autoestima", "Crise existencial", "Busca por significado", "Crescimento pessoal", "Dificuldades de autoexpressão"]
}, {
  id: "4",
  ...
  benefits: ["Melhoria na comunicação familiar", "Identificação de padrões relacionais prejudiciais", "Fortalecimento das relações saudáveis", "Resolução de conflitos interpessoais"],
  idealFor: ["Conflitos familiares", "Problemas conjugais", "Dificuldades parentais", "Crianças e adolescentes", "Transições familiares"]
}, {
  id: "5",
  name: "Mindfulness",
  ...
  benefits: ["Redução de estresse e ansiedade", "Melhora na concentração e atenção", "Desenvolvimento de regulação emocional", "Maior consciência corporal e mental"],
  idealFor: ["Estresse crônico", "Ansiedade", "Prevenção de recaída em depressão", "Dor crônica", "Transtornos alimentares"]
}, {
  id: "6",
  name: "Arteterapia",
  ...
  benefits: ["Expressão não-verbal de emoções complexas", "Acesso a conteúdos inconscientes", "Desenvolvimento da criatividade e autoexpressão", "Redução de estresse e ansiedade"],
  idealFor: ["Trauma", "Transtornos emocionais", "Crianças e adolescentes", "Idosos", "Pessoas com dificuldade de expressão verbal"]
}];
const Therapies = () => {
  ...
  useEffect(() => {
    const results = therapyTypes.filter(therapy => therapy.name.toLowerCase().includes(searchTerm.toLowerCase()) || therapy.shortDescription.toLowerCase().includes(searchTerm.toLowerCase()) || therapy.idealFor.some(condition => condition.toLowerCase().includes(searchTerm.toLowerCase())));
    if (activeTab !== "all") {
  ...
  return <div className="min-h-screen bg-teal-900 text-white flex flex-col">
      <Header />
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-10">
          ...
          <div className="mb-10">
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between mb-6">
              <Input placeholder="Buscar por nome ou condição..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="max-w-md bg-teal-800/50 border-lavender-400/30 text-white placeholder:text-white/50" />
              <Button onClick={() => setSearchTerm("")} variant="outline" className="border-lavender-400/50 hover:bg-lavender-400/20">
                Limpar busca
              </Button>
            </div>
            ...
            <Tabs defaultValue="all" value={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="w-full flex flex-wrap bg-teal-800/30 border border-lavender-400/20">
                <TabsTrigger value="all" className="flex-1 data-[state=active]:bg-lavender-400 data-[state=active]:text-teal-900">
                  Todas
                </TabsTrigger>
                {therapyTypes.map(therapy => <TabsTrigger key={therapy.id} value={therapy.id} className="flex-1 data-[state=active]:bg-lavender-400 data-[state=active]:text-teal-900">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {filteredTherapies.length > 0 ? filteredTherapies.map(therapy => <Card key={therapy.id} className="bg-teal-800/40 hover:bg-teal-800/50 border-lavender-400/20 transition-all duration-300">
                  <CardContent className="p-6">
                    ...
                    <div className="mb-6">
                      ...
                      <ul className="list-disc list-inside text-white/80 space-y-1 pl-2">
                        {therapy.benefits.map((benefit, index) => <li key={index}>{benefit}</li>)}
                      </ul>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {therapy.idealFor.map((condition, index) => <span key={index} className="text-sm bg-teal-700/50 px-3 py-1 rounded-full border border-lavender-400/30">
                            {condition}
                          </span>)}
                      </div>
                    </div>
                ...
                </Card>) : <div className="col-span-2 text-center py-12 bg-teal-800/20 rounded-lg">
                <p className="text-lg text-white/70">Nenhuma terapia encontrada com os critérios de busca.</p>
                <Button onClick={() => {
              setSearchTerm("");
              setActiveTab("all");
            }} variant="link" className="text-lavender-400 hover:text-lavender-300 mt-2">
                  Limpar filtros
                </Button>
              </div>}
          </div>
          ...
          {/* CTA Section */}
          <div className="mt-16 bg-gradient-to-r from-teal-800/50 to-lavender-500/20 p-8 rounded-xl border border-lavender-400/30 bg-lavender-400">
